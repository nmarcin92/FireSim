...
public abstract class AbstractLayer<T extends AbstractField> {
    ...
    public AbstractLayer(int sizeX, int sizeY, DynamicState simulationState) {
        ...
        layer = ArrayTable.create(ContiguousSet.create(Range.closed(0, sizeY), DiscreteDomain.integers()).asList(),
                ContiguousSet.create(Range.closed(0, sizeX), DiscreteDomain.integers()).asList());
    ...
    public void updateFields(LayerContainer layerContainer) {
        for (Table.Cell<Integer, Integer, T> cell : layer.cellSet()) {
            cell.getValue().update(layerContainer);
        }
    }
